  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="py-8 text-center border-b border-gray-700 flex-shrink-0">
          ...
          <div className="text-sm text-gray-500">
            Mood: {jarvis.state.mood_score?.toFixed(1) || '0.0'}/10
          </div>
        </div>

        {/* Conversation History */}
        <div className="flex-1 overflow-y-auto px-8 py-6 space-y-4">
          {!jarvis.state.conversation_history || jarvis.state.conversation_history.length === 0 ? (
                ...
                try {
                  ...
                  recognition.onresult = (event) => {
                    const transcript = Array.from(event.results)
                      ...
                      .join('')
                    if (transcript.trim()) jarvis.sendInput(transcript)
                  }
      ...
      <div className="border-t border-gray-700 px-8 py-3 bg-black/60 flex justify-between text-xs text-gray-500 flex-shrink-0">
        <div>💰 €{jarvis.state.api_cost_today?.toFixed(4) || '0.0000'}</div>
        <div>💵 €{jarvis.state.today_spent?.toFixed(2) || '0.00'}</div>
